...
class TempPlot:
    ...
    def plotNow(self,werte):
        ...
        #dates = drange(firstdate, lastdate-(delta*6), delta) ab 2Uhr nachts -60 min?
        #dates = drange(firstdate, lastdate+(delta*2), delta) um 0Uhr + 20Min
        #dates = drange(firstdate, lastdate, delta)
        #dates = drange(firstdate, firstdate+(delta*(len(werte))), delta)
