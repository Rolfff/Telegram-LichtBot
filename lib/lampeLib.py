...
# Configure the count of pixels from new config
config = Config()
PIXEL_COUNT = config.get_led_pixel_count()
ANZSTEGE = config.get_led_num_segments()
#Reinfolge der LED-Adressen pro Steg im Uhrzeigersin
PIXEL_MAP = config.get_led_pixel_map()
BOTTOM_LED = config.get_led_bottom_led()
...
class light:
    ...
    bottomled = None
    pixels = None
    # Global singleton for lightmatrix and lightlist
    OneLightmatrix = None
    OneLightlist = None
    ...
    def __init__(self):
        ...
        if light.OneLightmatrix is None or light.OneLightlist is None:
            ...
            for x in range(len(PIXEL_MAP)):
                ...
                if int(len(PIXEL_MAP)/ANZSTEGE) == len(tmp):
                    ...
                    tmp = []
            light.OneLightmatrix = self.lightmatrix
            light.OneLightlist = self.lightlist
            
        else:
            self.lightmatrix = light.OneLightmatrix
            self.lightlist = light.OneLightlist
        #self.pixels.clear()
    ...
    def getWait(self, wait=None):
        if wait is None:
            wait = config.get_led_default_speed()
        return wait
    
    def stopThread(self):
        # Thread handling removed for simplicity - can be added back if needed
        pass
